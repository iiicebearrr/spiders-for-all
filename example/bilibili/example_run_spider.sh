...
# Author spider, which is a littele different from others
...
#       You can also use `download-by-author` command to avoid this
...
${BASE} bilibili run-spider author -p mid 31283043 -p total 1 -w "mid=31283043 limit 3" -s /tmp/bilibili_author_videos -d

# example 4: crawl all videos of some author
# NOTE: This may take a long time due to the risk control strategy of bilibili api
